...
class Scheduler {
    ...
    start() {
        ...
        // 🏆 THỨ 7 & CHỦ NHẬT 7h55 - Gọi vào đua đội
        cron.schedule("55 7 * * 0,6", () => {
            this.sendToAllThreads(
                "🏆🔥 CỒN 5 PHÚT NHÉ! 🔥🏆\n\n" +
            ...
            logger("✅ Gửi tin nhắn gọi vào đua đội 7h55", "SCHEDULER");
        }, { timezone: "Asia/Ho_Chi_Minh" });
        ...
        this.threadIDs.forEach(threadID => {
            try {
                this.api.sendMessage(message, threadID, (err) => {
                    if (err) {
                        logger(`❌ Lỗi gửi tin nhắn tới ${threadID}: ${err.message}`, "SCHEDULER");
                    }
                });
            } catch (error) {
                logger(`❌ Exception khi gửi tin nhắn: ${error.message}`, "SCHEDULER");
            }
        });
    }
}
