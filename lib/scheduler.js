...
class Scheduler {
    ...
    start() {
        ...
        // 🏆 THỨ 7 & CHỦ NHẬT 7h55 - Gọi vào đua đội (CÓ TAG EVERYONE)
        cron.schedule(
            "55 7 * * 0,6",
            () => {
                ...
                logger("✅ Gửi tin nhắn (CÓ TAG EVERYONE) gọi vào đua đội 7h55", "SCHEDULER");
            },
            { timezone: "Asia/Ho_Chi_Minh" },
        );
        ...
        this.threadIDs.forEach((threadID) => {
            try {
                this.api.getThreadInfo(threadID, (err, threadInfo) => {
                    if (err) {
                        logger(`❌ Lỗi lấy info nhóm ${threadID}: ${err.message}`, "SCHEDULER");
                        return;
                    }
