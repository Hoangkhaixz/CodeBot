        ...
        try {
            const threads = await new Promise((resolve, reject) => {
                this.api.getThreadList(100, null, (err, threads) => {
                    if (err) {
                        logger(
                            `⚠️ Không thể lấy live threads từ API: ${err.message || err}`,
                            "SCHEDULER",
                        );
                        return resolve([]);
                    }
            ...
            if (threads && threads.length > 0) {
                ...
                logger(
                    `✅ Tải ${this.liveThreads.length} nhóm live từ Facebook`,
                    "SCHEDULER",
                );

                // Merge database threads và live threads
                const mergedIDs = [
                    ...new Set([...this.threadIDs, ...this.liveThreads]),
                ];
                this.threadIDs = mergedIDs;
                logger(
                    `✅ Tổng ${mergedIDs.length} nhóm (database + live)`,
                    "SCHEDULER",
                );
            } else {
                logger(
                    `ℹ️ Không có live threads mới, dùng database threads (${this.threadIDs.length})`,
                    "SCHEDULER",
                );
            }
        } catch (err) {
            logger(
                `⚠️ Exception lấy live threads: ${err.message || err}`,
                "SCHEDULER",
            );
        }
    }
