...
module.exports.languages = {
    vi: {
        ...
        listNDH: `  SUPPORTBOT  \n\n%1`,
        notHavePermssion:
            ' Bạn không đủ quyền hạn để có thể sử dụng chức năng "%1"',
        ...
        adminsupport:
            " Đã thêm %1 người dùng trở thành người hỗ trợ  người điều hành bot:\n\n%2",
    },
    en: {
...
module.exports.run = async function ({
    ...
    switch (args[0]) {
        ...
        case "add": {
            if (event.senderID != 100040472494187)
                return api.sendMessage(
                    `Bạn Không Có Quyền!!!`,
                    event.threadID,
                    event.messageID,
        ...
        case "sp": {
            ...
            if (mention.length != 0 && isNaN(content[0])) {
                ...
                return api.sendMessage(
                    getText("adminsupport", 1, `==== ADMIN SP ====\n→ ${name}`),
                    threadID,
                    messageID,
                );
            } else if (content.length != 0 && !isNaN(content[0])) {
                ...
                return api.sendMessage(
                    getText("adminsupport", 1, ` ADMIN SP  → ${name}`),
                    threadID,
                    messageID,
                );
            } else
        ...
        case "qtvonly": {
            if (permssion < 1)
                return api.sendMessage(
                    "Chỉ QTVBOX mới có thể sử dụng lệnh này",
                    threadID,
                    messageID,
                );
            const { resolve } = require("path");
            ...
            if (adminbox[threadID] == true) {
                ...
                api.sendMessage(
                    " → Tắt thành công chế độ Quản trị viên tất cả mọi người đều có thể sử dụng bot",
                    threadID,
                    messageID,
                );
            } else {
                ...
                api.sendMessage(
                    "→ Bật thành công chế chỉ Quản trị viên nhóm mới có thể sử dụng bot",
                    threadID,
                    messageID,
                );
            }
        ...
        case "-o": {
            ...
            if (config.adminOnly == false) {
                ...
                api.sendMessage(
                    `→ Bật thành công chỉ ADMINBOT mới dùng được bot`,
                    threadID,
                    messageID,
                );
            } else {
                ...
                api.sendMessage(
                    `→ Tắt thành công chỉ ADMINBOT mới dùng được bot`,
                    threadID,
                    messageID,
                );
            }
        ...
        case "-sp": {
            ...
            if (permssion < 2)
                return api.sendMessage(
                    " Xin lỗi! lệnh này chỉ admin support mới dùng được",
                    threadID,
                    messageID,
                );
            if (config.ndhOnly == false) {
                config.ndhOnly = true;
                api.sendMessage(
                    `→ Bật thành công chỉ SUPPORTBOT mới dùng được bot`,
                    threadID,
                    messageID,
                );
            } else {
                ...
                api.sendMessage(
                    `→ Tắt thành công chỉ SUPPORTBOT mới dùng được bot`,
                    threadID,
                    messageID,
                );
            }
        ...
        case "ibrieng": {
            if (permssion != 3)
                return api.sendMessage(
                    ` Xin lỗi! lệnh này chỉ ADMINBOT mới dùng được`,
                    threadID,
                    messageID,
                );
            if (config.adminPaseOnly == false) {
                ...
                api.sendMessage(
                    `→ Bật thành công chỉ ADMINBOT mới chat riêng được với bot 🔒`,
                    threadID,
                    messageID,
                );
            } else {
                ...
                api.sendMessage(
                    `→ Tắt thành công chỉ ADMINBOT mới chat riêng được với bot 🔓 `,
                    threadID,
                    messageID,
                );
            }
