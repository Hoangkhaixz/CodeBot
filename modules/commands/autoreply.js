...
module.exports.config = {
    ...
    credits: "TatsuYTB",
    description:
        "Thiết lập tự động trả lời tin nhắn cho từng nhóm, hỗ trợ hình ảnh và video",
...
const countKeywordsByType = (data, threadID, senderID, type) => {
    ...
    return Object.values(data[threadID]).filter(
        (k) => k.senderID === senderID && k.type === type,
    ).length;
};
        ...
        if (replyData.type === "text") {
            ...
            if (!fs.existsSync(replyData.path)) {
                return api.sendMessage(
                    "⚠️ Tệp đính kèm không tồn tại hoặc đã bị xóa.",
                    threadID,
                    messageID,
                );
            }
    ...
    switch (action) {
        ...
        case "addimage": {
            ...
            if (action === "add" && (!reply || reply.length === 0))
                return api.sendMessage(
                    "Vui lòng nhập cú pháp: từ khóa | nội dung",
                    threadID,
                    messageID,
                );
            ...
            let isFile = false,
                attachment = null;
            if (
                (action === "addvideo" || action === "addimage") &&
                ...
                if (action === "addvideo" && attachment.type !== "video")
                    return api.sendMessage(
                        "Tệp đính kèm không phải video.",
                        threadID,
                        messageID,
                    );
                if (action === "addimage" && attachment.type !== "photo")
                    return api.sendMessage(
                        "Tệp đính kèm không phải hình ảnh.",
                        threadID,
                        messageID,
                    );
            }
            if ((action === "addvideo" || action === "addimage") && !attachment)
                return api.sendMessage(
                    "Vui lòng reply kèm một tệp đính kèm phù hợp.",
                    threadID,
                    messageID,
                );

            if (!data[threadID]) data[threadID] = {};
            const textCount = countKeywordsByType(
                data,
                threadID,
                senderID,
                "text",
            );
            const imageCount = countKeywordsByType(
                data,
                threadID,
                senderID,
                "image",
            );
            const videoCount = countKeywordsByType(
                data,
                threadID,
                senderID,
                "video",
            ...
            if (action === "add" && textCount >= 10)
                return api.sendMessage(
                    "Bạn đã đạt giới hạn 10 từ khóa gửi văn bản.",
                    threadID,
                    messageID,
                );
            if (action === "addimage" && imageCount >= 5)
                return api.sendMessage(
                    "Bạn đã đạt giới hạn 5 từ khóa gửi hình ảnh.",
                    threadID,
                    messageID,
                );
            if (action === "addvideo" && videoCount >= 3)
                return api.sendMessage(
                    "Bạn đã đạt giới hạn 3 từ khóa gửi video.",
                    threadID,
                    messageID,
                );

            const extension = isFile
                ? getFileExtension(attachment.url, attachment.type)
                : null;
            const uniqueFileName = isFile
                ? getUniqueFileName(attachment.type, attachment.id, extension)
                : null;
            const filePath = isFile
                ? `${__dirname}/data/autoreply/${uniqueFileName}`
                : null;
            ...
            if (isFile) {
                try {
                    ...
                    fs.writeFileSync(path, JSON.stringify(data, null, 2));
                    return api.sendMessage(
                        `Đã thêm từ khóa "${key}" kèm ${attachment.type === "video" ? "video" : "ảnh"}.`,
                        threadID,
                        messageID,
                    );
                } catch (err) {
        ...
        case "list": {
            if (!data[threadID] || Object.keys(data[threadID]).length === 0)
                return api.sendMessage(
                    "Hiện không có từ khóa nào được thiết lập.",
                    threadID,
                    messageID,
                );
