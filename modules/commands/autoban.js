...
module.exports.config = {
  ...
  credits: "TatsuYTB",
  description:
    "Tự động ban người dùng khi họ chửi bot, ngoại trừ những người có ID được chỉ định",
...
module.exports.handleEvent = async function ({ event, api, Users, Threads }) {
  ...
  if (senderID == global.data.botID || exemptIDs.includes(senderID.toString()))
    return;
  ...
  for (let keyword of keywords) {
    if (body.toLowerCase().includes(keyword.toLowerCase())) {
      ...
      let threadName =
        threadInfo.threadInfo.threadName || "Tên nhóm không xác định";
      ...
      if (!userWarnings[senderID]) {
        ...
        api.sendMessage(
          `⚠️ Cảnh báo lần 1: ${name}, nếu tiếp tục chửi bot, bạn sẽ bị cấm.`,
          threadID,
        );
      } else if (userWarnings[senderID] === 1) {
        userWarnings[senderID] = 2;
        api.sendMessage(
          `⚠️ Cảnh báo lần 2: ${name}, nếu còn tiếp tục, bạn sẽ bị cấm.`,
          threadID,
        );
